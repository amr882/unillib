...
class _BorrowedBooksSectionState extends State<BorrowedBooksSection> {
  ...
  Future<void> _fetchBooks() async {
    ...
    // In a real app we might want to do a batch fetch or rely on catalog
    for (final record in records) {
      // Find the book in catalog first, otherwise we might just construct a minimal Book
      Book? book = catalog.allBooks.cast<Book?>().firstWhere(
        (b) => b?.id == record.bookId,
        orElse: () => null,
      );

      // If not fully loaded, make a dummy book just for display based on record data
      book ??= Book(
        id: record.bookId,
        rawId: record.bookId,
        ...
        tags: [],
        reservedBy: [],
        borrowedBy: [record.userId], // to indicate borrowed
        reservations: {},
  ...
  /// Format duration as "X days Y hours" or "XXh YYm"
  ...
  Color _getTimerColor(Duration d) {
    if (d == Duration.zero) return Colors.red;
    if (d.inHours < 4) return Colors.red;
    if (d.inHours < 12) return Colors.orange;
    return Colors.green;
  }

  /// Get icon based on remaining time
  IconData _getTimerIcon(Duration d) {
    if (d == Duration.zero) return Icons.error_outline_rounded;
    if (d.inHours < 4) return Icons.warning_amber_rounded;
  ...
  Widget _buildCountdownChip(_BorrowItem item) {
    ...
    if (item.record.status == BorrowStatus.activeBorrow) {
