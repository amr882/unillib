...
class BorrowService {
  ...
  Future<BorrowScanResult> confirmPickup(String borrowId) async {
    try {
      ...
      if (!doc.exists) {
        return BorrowScanResult(success: false, error: 'Record not found.');
      }

      final record = BorrowRecord.fromMap(doc.data()!);
      if (record.status != BorrowStatus.pendingPickup) {
        return BorrowScanResult(
          success: false,
          error: 'Book already picked up or cancelled.',
        );
      }
    ...
    try {
      ...
      try {
        ...
        NotificationService().showNotification(
          ...
          title: 'Book Returned! 📚',
          body:
              'Your return for "${record.bookTitle}" has been successfully confirmed. Thank you!',
        );
      } catch (e) {
